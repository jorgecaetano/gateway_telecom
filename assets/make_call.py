...
def fsapi(session, stream, env, args):
    ...
    dial_string, destination, cpf_cnpj, process_id, timeout, amount = _args
    ...
    for i in range(amount):
        uniqueid = uuid4()
        variables = '{origination_uuid=%s,CALL_ID=%s,DESTINATION=%s,CPF_CNPJ=%s,PROCESS_ID=%s,ignore_early_media=true,sip_sticky_contact=true,absolute_codec_string=PCMA}[leg_timeout=%s]' % (uniqueid, uniqueid, destination, cpf_cnpj, process_id, timeout)
