...
def fsapi(session, stream, env, args):
    ...
    for i in range(amount):
        uniqueid = uuid4()
        variables = '{origination_uuid=%s,CALL_ID=%s,DESTINATION=%s,PROCESS_ID=%s,ignore_early_media=true,sip_sticky_contact=true,absolute_codec_string=PCMA}[leg_timeout=%s]' % (
        uniqueid, uniqueid, destination, process_id, timeout)
